...
// @ts-ignore - ContractTypesMap is generated by hardhat-viem
...
async function signPermitERC2612(params: {
    ...
    let version = '1';
    try {
        const v = await (token.read as unknown as { version?: () => Promise<string> }).version?.();
        if (typeof v === 'string') version = v;
    } catch {
        // version() may not exist on all tokens
    }
