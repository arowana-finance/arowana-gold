...
contract InitializableERC20 is ERC20Upgradeable, ERC20BurnableUpgradeable, ERC20PermitUpgradeable {
    ...
    /// @dev Parent initializer — callable only from a child contract's `initializer`
    ///      (e.g. GoldToken.initializeGoldToken). `onlyInitializing` makes the guard
    ///      explicit: previously a direct post-init call was blocked only incidentally
    ///      by the OZ parents' own `onlyInitializing` guards (upgrades-core error-001).
    function initializeToken(string memory name_, string memory symbol_, uint8 decimals_, uint256 supply_)
        public
        onlyInitializing
    {
